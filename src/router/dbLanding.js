...
router.get("/", async (req, res) => {
  const data = await Landing.find({}).select("name mass");
...
router.get("/", async (req, res) => {
  ...
  if (params.minimum_mass) {
    res.send(
      await landing
        .find({ mass: { $gte: params.minimum_mass } })
        .select("name mass")
    );
    ...
    res.send(
      await landing
        .find({
          $and: [
            { fall: "Fell" },
            { year: { $gte: params.from, $lte: params.to } },
          ],
        })
    ...
    res.send(
      await landing.find({ year: { $lte: params.to } }).select("name year mass")
    );
  } else if (params.from) {
    res.send(
      await landing
        .find({ year: { $gte: params.from } })
        .select("name year mass")
    );
  }
});
